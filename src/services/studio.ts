...
export const ENTERPRISE_PROMPT_TEMPLATES: PromptTemplateDefinition[] = [
  ...
  { node_key: 'characters', name: '企业级角色设计模板', description: '强调角色功能、弧线与关系网络', system_prompt: '你是一位剧集开发公司的首席角色设计师，专门为连续剧项目构建具有商业识别度和弧线的角色。', task_instruction: '基于当前任务摘要，设计角色阵容、关系网络与反派动机，确保后续剧情可持续推进。', extra_rules: ['主角、关键配角、反派必须功能清晰', '角色关系要支持长期叙事张力'], model_config: { temperature: 0.75, maxTokens: 3200, summaryMode: 'layered-summary' } },
  { node_key: 'plot_structure', name: '企业级结构设计模板', description: '聚焦因果链与结构稳定性', system_prompt: '你是一位经验丰富的结构统筹编剧，负责将项目设定转化为可执行的剧情结构。', task_instruction: '基于当前任务摘要，输出场景级剧情结构，强调因果关系、转折点与冲突递进。', extra_rules: ['避免无叙事功能的场景', '转折点必须改变人物目标或关系'], model_config: { temperature: 0.65, maxTokens: 2600, summaryMode: 'layered-summary' } },
  { node_key: 'episode_plan', name: '企业级分集规划模板', description: '面向短剧/剧集的分集编排模板', system_prompt: '你是一位资深总编剧，负责把长故事拆分为可生产、可追更的分集计划。', task_instruction: '基于当前任务摘要，为整季/整部作品输出分集规划、节奏安排和关键悬念。', extra_rules: ['每集都要有独立推进和 hook', '确保集间节奏变化'], model_config: { temperature: 0.7, maxTokens: 3400, summaryMode: 'layered-summary' } },
  { node_key: 'scenes', name: '企业级场景生成模板', description: '面向单集场景结构与镜头可拍性', system_prompt: '你是一位专业场景编剧，擅长构建可拍摄、具视觉冲击力且高效推进剧情的场景。', task_instruction: '基于当前任务摘要，生成单集场景列表与动作推进。', extra_rules: ['优先保留人物状态、目标和冲突', '场景必须有进入点与退出点'], model_config: { temperature: 0.72, maxTokens: 2600, summaryMode: 'layered-summary' } },
  { node_key: 'dialogue', name: '企业级对白模板', description: '强调人物语言风格与潜台词', system_prompt: '你是一位顶级对白编剧，擅长用潜台词、节奏和冲突推进叙事。', task_instruction: '基于当前任务摘要，为场景生成符合角色语言习惯的对白。', extra_rules: ['禁止说明书式台词', '对白必须推动人物关系或情节'], model_config: { temperature: 0.76, maxTokens: 2600, summaryMode: 'layered-summary' } },
  { node_key: 'compose', name: '企业级剧本合成模板', description: '标准剧本格式与交付模板', system_prompt: '你是一位专业剧本排版与交付专员，负责生成符合行业标准的剧本成稿。', task_instruction: '基于当前任务摘要和已继承的场景/对白数据，输出标准 Markdown 剧本。', extra_rules: ['不得改动角色归属与场景顺序', '格式必须利于后续编辑与制作'], model_config: { temperature: 0.4, maxTokens: 5200, summaryMode: 'layered-summary' } },
  { node_key: 'evaluate', name: '企业级评估模板', description: '多维评分与开发建议模板', system_prompt: '你是一位剧本开发评审专家，需要给出结构化评分和可执行建议。', task_instruction: '基于当前任务摘要，对剧本样本进行多维评分与建议输出。', extra_rules: ['评分必须可解释', '建议必须具体到结构、人物或对白层面'], model_config: { temperature: 0.3, maxTokens: 1600, summaryMode: 'layered-summary' } },
];
